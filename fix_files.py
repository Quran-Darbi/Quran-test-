...
# تحديث تسمية الزر (أغسطس ٢٠٢٦): «اختبر تلاوتك» → «اختبر حفظك» — الاسم
# القديم كان بيوهم المستخدم إن الموقع بيتحقق من التشكيل/التجويد من
# الصوت، وده مش صحيح (راجع مذكرة هند). الدالة دي بترقّع النص بس في
# الأزرار اللي اتضافت من نسخ سابقة من _RECIT_BTN_TPL، وجراحية/idempotent:
# لو النص الجديد موجود بالفعل مابتعملش حاجة.
def retitle_recitation_button(path, out):
    OLD_LABEL = '🎤 اختبر تلاوتك'
    NEW_LABEL = '🎤 اختبر حفظك'
    if OLD_LABEL not in out:
        return out, False
    fn = os.path.basename(path)
    new = out.replace(OLD_LABEL, NEW_LABEL)
    RECIT_BTN_RETITLED.append(fn)
    return new, True
...
def fix_file(path):
    ...
    out, _recbtn_retitled = retitle_recitation_button(path, out)
...
def main():
    ...
    if RECIT_BTN_SKIPPED:
        ...
        for fn2, why in RECIT_BTN_SKIPPED:
            print('  -', fn2, ':', why)
    print('اتجدد نصه (اختبر تلاوتك → اختبر حفظك): %d ملف' % len(RECIT_BTN_RETITLED))
    if RECIT_BTN_RETITLED:
        print('  ', ' · '.join(RECIT_BTN_RETITLED[:20]))
