...
COMPACT_CSS = (
    ...
    ".info-card{padding:16px 18px 14px;margin-bottom:12px;}"
    ".info-card::before,.info-card::after{top:12px;}"
    ".surah-title{font-size:24px;margin-bottom:2px;}"
    ".ayat-range{margin-bottom:12px;}"
    ".page-info{font-size:13.5px;margin-bottom:0;}"
    ".rec-inline-btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;"
    "background:var(--surface2);border:1.5px solid var(--border);border-radius:12px;"
    "padding:9px 18px;margin-bottom:12px;font-size:15px;font-family:inherit;"
    "color:var(--accent);text-decoration:none;min-height:44px;box-sizing:border-box;}"
    ".stat-badge{padding:6px 14px;font-size:13px;min-width:72px;}"
    ".level-card{padding:16px 16px 16px;margin-bottom:12px;}"
    ".level-title{font-size:20px;margin-bottom:4px;}"
    ".level-sub{font-size:13px;margin-bottom:14px;}"
    ".levels-grid{gap:7px;margin-bottom:14px;}"
    ".level-btn{padding:10px 6px;}"
    ".level-icon{font-size:24px;height:26px;margin-bottom:4px;}"
    ".level-name{font-size:14px;margin-bottom:4px;}"
    "/*COMPACT-END*/\n"
)

# كرت العنوان: سطر الآيات وسطر وجه/صفحة منفصلين — بندمجهم في سطر واحد
MERGE_INFO_RE = re.compile(
    r'<div class="ayat-range">(.*?)</div>\s*\n?\s*<div class="page-info">(.*?)</div>'
)
# «اختبار الحفظ |» بادئة مكررة في كل صفحة — بتتشال عند الدمج عشان
# السطر الواحد يدخل من غير لف على الموبايل
INFO_PREFIX_RE = re.compile(r'^\s*اختبار الحفظ\s*[|\u2014\-·]?\s*')

# زر التلاوة ككرت مستقل (بغلاف div بهامش) — بينتقل جوّه كرت العنوان
REC_CARD_RE = re.compile(
    r'<div style="margin:0 16px 16px;">\s*<a href="(recitation\.html\?surah=[^"]+)"'
    r'[^>]*>\s*🎤 اختبر تلاوتك\s*</a>\s*</div>\s*\n?', re.S
)
...
_SR_SHOW_RE = re.compile(
    r"(document\.getElementById\('quiz-area'\)\.style\.display='block';)(\s*showQuestion\(\);)"
)
_SR_HIDE_RE = re.compile(
    r"(document\.getElementById\('level-card'\)\.style\.display='block';)"
)

LEVEL_DESC_TEXTS = (
    ('<span class="level-desc">اختيار من متعدد</span>',
     '<span class="level-desc">اختر الإجابة الصحيحة</span>'),
    ('<span class="level-desc">إكمال فراغ</span>',
     '<span class="level-desc">أكمل الفراغ</span>'),
)


def compact_first_screen(path, out):
    """بتخلي أول شاشة (كرت العنوان + اختيار المستوى) تدخل كاملة في شاشة
    الموبايل من غير scroll، من غير ما تلمس الألوان ولا هوية الموقع ولا
    مساحات الضغط (كل زر يفضل ≥٤٤px). خمس حتت، كلها idempotent:

      أ. شريط الإحصائيات (٠ صحيح/٠ خطأ/السؤال ١) يتخفي على شاشة اختيار
         المستوى — أرقامه فاضية وملهاش معنى قبل بدء الاختبار — ويظهر
         أول ما الاختبار يبدأ (startQuiz/resumeQuiz) ويتخفي تاني مع
         الرجوع للمستويات (returnToLevels/retryQuiz).
      ب. زر 🎤 اختبر تلاوتك ينتقل من كرت مستقل لجوّه كرت العنوان.
      ج. سطر «الآيات …» + سطر «وجه … صفحة …» يتدمجوا في سطر واحد
         (بادئة «اختبار الحفظ |» بتتشال — مكررة في كل صفحة).
      د. ضغط المسافات والأحجام عبر كتلة CSS واحدة في آخر الـstyle —
         بتيجي بعد قواعد fix_level_card_alignment فبتغلب عليها من غير
         ما نلمسها.
      هـ. وصف المستويين السهل/المتوسط بصياغة أوضح للمستخدم.

    المجموع ~٢٠٠px. (أغسطس ٢٠٢٦)"""
    changed = False

    # ── هـ. نصوص وصف المستويات ──
    for old_t, new_t in LEVEL_DESC_TEXTS:
        if old_t in out:
            out = out.replace(old_t, new_t, 1)
            changed = True

    # الباقي بيخص شاشة اختيار المستوى — الملفات اللي مافيهاش كرت
    # عنوان (index/recitation) بتتخطى
    if '<div class="info-card">' not in out:
        return out, changed

    # ── ج. دمج سطري الآيات ووجه/صفحة ──
    m = MERGE_INFO_RE.search(out)
    if m:
        rng = m.group(1).strip()
        rest = INFO_PREFIX_RE.sub('', m.group(2).strip())
        merged = ('<div class="ayat-range">' + rng +
                  ' <span class="page-info">· ' + rest + '</span></div>')
        out = out[:m.start()] + merged + out[m.end():]
        changed = True

    # ── أ. شريط الإحصائيات ──
    if '<div class="stats-row">' in out:
        out = out.replace(
            '<div class="stats-row">',
            '<div class="stats-row" id="stats-row" style="display:none;">', 1)
        changed = True
    if 'stats-row' in out and _SR_SHOW not in out:
        new_out, n = _SR_SHOW_RE.subn(lambda mm: mm.group(1) + _SR_SHOW + mm.group(2), out)
        if n:
            out, changed = new_out, True
    if 'stats-row' in out and _SR_HIDE not in out:
        new_out, n = _SR_HIDE_RE.subn(lambda mm: mm.group(1) + _SR_HIDE, out)
        if n:
            out, changed = new_out, True

    # ── ب. زر التلاوة جوّه كرت العنوان ──
    mrec = REC_CARD_RE.search(out)
    if mrec and 'class="rec-inline-btn"' not in out:
        href = mrec.group(1)
        out = out[:mrec.start()] + out[mrec.end():]
        inline = ('  <a href="' + href + '" class="rec-inline-btn">'
                  '🎤 اختبر تلاوتك</a>\n')
        anchor = '  <div class="stats-row"'
        if anchor in out:
            out = out.replace(anchor, inline + anchor, 1)
        else:
            out = out.replace('</div>\n<div id="resume-banner"',
                              inline + '</div>\n<div id="resume-banner"', 1)
        changed = True

    # ── د. كتلة الضغط في آخر الـstyle ──
    if COMPACT_MARK not in out and '</style>' in out:
        i = out.rindex('</style>')
        out = out[:i] + COMPACT_CSS + out[i:]
        changed = True
...
def fix_file(path):
    ...
    # ضغط أول شاشة (كرت العنوان + اختيار المستوى) عشان تدخل كاملة في
    # شاشة الموبايل من غير scroll — لازم بعد fix_level_card_alignment
    # وfix_hard_level_desc عشان كتلة CSS بتاعتها تيجي بعدهم في الـstyle
    out, _compacted = compact_first_screen(path, out)
