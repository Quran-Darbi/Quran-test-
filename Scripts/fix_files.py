...
def fix_levels_wording(path, out):
    ...
    for old in (HERO_SUB_OLD, HERO_SUB_PREV):
        if old in out:
            out = out.replace(old, HERO_SUB_NEW, 1)
            changed = True
            break
...
WA_WHITELIST = ("سجد|قترب|دخل|دعو|ذكر|رحم|ستغفر|ستغن|غفر|عف|نحر|تق|ختلاف|"
                ...
                "علم|رزق|جعل|خش|شكر|نظر|بعث|قتل|نصر|ستشهد|جتب|متاز")
...
def _nf_one(body):
    ...
    # سبتمبر ٢٠٢٦: الصيغة "ئ/ؤ → ا" (كل الأشكال اللي قبلها) اتضح إنها غلط
    # هي كمان — شئ (ش+ئ، من شيء) كانت بتتحول لـ"شا" بدل "شي" فتُحسب خطأ
    # رغم إن "شىء" (بحرف ى منفصل) بتتحسب صح؛ ويستهزؤن بتتحول لـ"يستهزان"
    # بدل "يستهزون". القاعدة الصحيحة: ئ/ؤ كرسي حرف علة حامل همزة — الهمزة
    # تختفي والكرسي يفضل حرف علة (ي/و) إلا لو ئ جاية قبل و مباشرة (متكئون)
    # وقتها الوحدة كلها بتتحذف زي رسم القرآن (كشيدة+همزة) عشان محدش
    # حرف زيادة عن رسم الأصل.
    _NEW_HAMZA = r"replace(/ئ(?=و)/g,'').replace(/ئ/g,'ي').replace(/ؤ/g,'و').replace(/ء/g,'')"
    if r"replace(/[ئؤ]/g,'ء').replace(/ء/g,'')" in body:
        body = body.replace(r"replace(/[ئؤ]/g,'ء').replace(/ء/g,'')", _NEW_HAMZA)
        done.append('ئؤ→ي/و')
    if r"replace(/[ءئؤ]/g,'')" in body:
        body = body.replace(r"replace(/[ءئؤ]/g,'')", _NEW_HAMZA)
        done.append('ئؤ→ي/و')
    if r"replace(/[ئؤ]/g,'ا').replace(/ء/g,'')" in body:
        body = body.replace(r"replace(/[ئؤ]/g,'ا').replace(/ء/g,'')", _NEW_HAMZA)
        done.append('ئؤ→ي/و')
    if r"replace(/ئ/g,'').replace(/ؤ/g,'ا').replace(/ء/g,'')" in body:
        body = body.replace(r"replace(/ئ/g,'').replace(/ؤ/g,'ا').replace(/ء/g,'')", _NEW_HAMZA)
        done.append('ئؤ→ي/و')
...
def fix_recitation_norm_hamza(path, out):
    ...
    سبتمبر ٢٠٢٦: الهدف اتغيّر تاني — [ئؤ]→ا (وكل الصيغ القديمة قبلها)
    اتضح غلط برضه (شئ↔شيء، يستهزؤن↔يستهزون). الصيغة الصحيحة الجديدة:
    ئ قبل و مباشرة تتحذف بالكامل، غير كده ئ→ي، ؤ→و.
    """
    new = r".replace(/ئ(?=و)/g,'').replace(/ئ/g,'ي').replace(/ؤ/g,'و').replace(/ء/g,'')"
    candidates = [
        r".replace(/[ئؤ]/g,'ء').replace(/ء/g,'')",
...
def fix_file(path):
    ...
    WA_LIST_2026_08 = r"سجد|قترب|دخل|دعو|ذكر|رحم|ستغفر|ستغن|غفر|عف|نحر|تق|ختلاف|مر[أا]|تبع|سمع|ستكبر|ستعين|ركع|صبر|صل|جتنب|هبط|ستبشر|ستقم|ضرب|عتصم|ئتلف|بتغ|حذر|شرب|صفح|تخذ|علم|رزق|جعل|خش|شكر|نظر|بعث|قتل|نصر|ستشهد|علم|رزق|جعل|خش|شكر|نظر|بعث|قتل|نصر|ستشهد|جتب|متاز"
